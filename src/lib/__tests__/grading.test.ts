...
import { runPreviewQualityCheck, runPublicationQualityCheck, runQualityCheck } from '../qualityCheck';
...
describe('grading and review contract', () => {
  ...
  it('allows publishing to promote an evidence-complete draft to verified', () => {
    const errors = runPublicationQualityCheck({
      verified: false,
      sections: [{
        type: 'essay',
        title: 'Writing',
        tasks: [{
          questions: [{
            ...
            stem: 'Write an essay.',
            verified: true,
            sourceReferences: [{ pageNumber: 1 }],
            answerRules: [{ standardAnswer: 'Reference answer', disputed: false }],
          }],
        }],
      }],
    });
